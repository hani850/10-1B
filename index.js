...
db.serialize(function () {
    db.run("CREATE TABLE IF NOT EXISTS User(id INTEGER PRIMARY KEY AUTOINCREMENT, fname TEXT, sname TEXT, email TEXT, date TEXT, q1 INTEGER, q2 INTEGER, q3 INTEGER, colour TEXT, comment TEXT)");
...
app.post('/submitsurvey', (req, res, next) => {
    ...
    console.log("outputting date: " + isoString);
...
app.get('/user', function (req, res) {
    ...
    html += 'There is a total of ' + submissionCount + ' surveys completed.</p>';
    ...
    db.all('SELECT * FROM User', function (err, rows) {
        ...
        if (rows.length === 0) {
            ...
            rows.forEach(function (row) {
                ...
                html += '<small>' + row.id + '  ' + row.fname + '  ' + row.sname + '</small></p>';
                html += '<div class="row"><div class="col-sm-6">';
                html += '<p class="text-primary mb-0">Comment</p><p><em>' + row.comment + '</em></p></div>';
                html += '<div class="col-sm-2"><p class="text-primary mb-0">Colour</p><div class="badge text-wrap" style="background-color:' + row.colour + '">' + row.colour + '</div></div>';
                html += '<div class="col-sm-4"><p class="text-primary mb-0">Results</p><div class="progress" role="progressbar" aria-label="Success example" aria-valuenow="' + row.q1 + `"`;
                html += 'aria-valuemin="1" aria-valuemax="5">Q1<div class="progress-bar text-bg-success" style="width:60%">' + row.q1 + '</div></div>';
                html += '<div class="progress" role="progressbar" aria-label="Success example" aria-valuenow="' + row.q2 + '" aria-valuemin="1" aria-valuemax="5">Q2<div class="progress-bar text-bg-success" style="width:20%">' + row.q2 + '</div></div>';
                html += '<div class="progress" role="progressbar" aria-label="Success example" aria-valuenow="' + row.q3 + '" aria-valuemin="1" aria-valuemax="5"> Q3<div class="progress-bar text-bg-success" style="width:' + (row.q3 / 5) * 100 + '%">' + row.q3 + '</div></div>';
                html += '</div></div><p class="text-secondary mb-1"><small>' + row.date + '</small></p></div>';
            });
        }
        html += '</div></main>';
